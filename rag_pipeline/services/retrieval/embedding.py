...
class EmbeddingService:
    """
    Generates dense vector embeddings from text using either
    Google Gemini Embedding API or a local sentence-transformers model.
    """
        ...
        if provider == 'gemini':
            ...
            if key and not key.startswith("YOUR_"):
                try:
                    ...
                    logger.info(f"Embedding service initialized with Gemini API model: {self.gemini_embedding_model}.")
    ...
    def _extract_gemini_embedding(self, result) -> np.ndarray:
        """Extracts embedding vector from various Gemini SDK response formats."""
